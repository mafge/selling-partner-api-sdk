...
/**
 * A &#x60;notificationType&#x60; specific filter. This object contains all of the currently
 * available filters and properties that you can use to define a &#x60;notificationType&#x60;
 * specific filter.
 */
@Schema(
    description =
        "A `notificationType` specific filter. This object contains all of the currently available"
            + " filters and properties that you can use to define a `notificationType` specific"
            + " filter.")
public class EventFilter extends AggregationFilter {
   ...
   * &#x60;notificationType&#x60;. This is used by the subscription service to determine the type of
   * event filter. Refer to [Notification Type
   * Values](https://developer-docs.amazon.com/sp-api/docs/notification-type-values) to determine if
   * an &#x60;eventFilterType&#x60; is supported.
   */
  ...
  public enum EventFilterTypeEnum {
    ...
    public static class Adapter extends TypeAdapter<EventFilterTypeEnum> {
      @Override
      public void write(final JsonWriter jsonWriter, final EventFilterTypeEnum enumeration)
          throws IOException {
  ...
  @Schema(
      required = true,
      description =
          "An `eventFilterType` value that is supported by the specific `notificationType`. This is"
              + " used by the subscription service to determine the type of event filter. Refer to"
              + " [Notification Type"
              + " Values](https://developer-docs.amazon.com/sp-api/docs/notification-type-values)"
              + " to determine if an `eventFilterType` is supported.")
