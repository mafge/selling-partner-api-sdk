...
public class ShipmentDetails {
  ...
  public enum ShipmentStatusEnum {
    ...
    public static class Adapter extends TypeAdapter<ShipmentStatusEnum> {
      @Override
      public void write(final JsonWriter jsonWriter, final ShipmentStatusEnum enumeration)
          throws IOException {
  ...
  /**
   * The date of the shipment&#x27;s departure from vendor&#x27;s location. Vendors send ASNs within
   * 30 minutes of departure from their warehouse/distribution center or six hours prior to the
   * appointment time at the Amazon destination warehouse. The shipped date mentioned in the
   * shipment confirmation cannot be in the future.
   *
   * @return shippedDate
   */
  @Schema(
      required = true,
      description =
          "The date of the shipment's departure from vendor's location. Vendors send ASNs within 30"
              + " minutes of departure from their warehouse/distribution center or six hours prior"
              + " to the appointment time at the Amazon destination warehouse. The shipped date"
              + " mentioned in the shipment confirmation cannot be in the future.")
  ...
  /**
   * The date on which the shipment is expected to reach the buyer&#x27;s warehouse. The date is
   * estimated based on the average transit time between the ship-from location and the destination.
   * Usually, the exact appointment time is unknown when creating the shipment confirmation and is
   * later provided by the buyer.
   *
  ...
  @Schema(
      description =
          "The date on which the shipment is expected to reach the buyer's warehouse. The date is"
              + " estimated based on the average transit time between the ship-from location and"
              + " the destination. Usually, the exact appointment time is unknown when creating the"
              + " shipment confirmation and is later provided by the buyer.")
  ...
  public int hashCode() {
    return Objects.hash(
        shippedDate, shipmentStatus, isPriorityShipment, vendorOrderNumber, estimatedDeliveryDate);
  }
