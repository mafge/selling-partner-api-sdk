...
public class AutomotiveApi {
   ...
   *
   * @param marketplaceId An identifier for the marketplace in which the resource operates.
   *     (required)
   * @param vehicleType An identifier for vehicle type. (required)
   * @param pageToken A token to fetch a certain page when there are multiple pages worth of
   *     results. (optional)
   * @param updatedAfter Date in ISO 8601 format, if provided only vehicles which are modified/added
   *     to Amazon&#x27;s catalog after this date will be returned. (optional)
  ...
  private okhttp3.Call getVehiclesValidateBeforeCall(
    ...
    if (marketplaceId == null) {
      throw new ApiException(
          "Missing the required parameter 'marketplaceId' when calling getVehicles(Async)");
    }
    // verify the required parameter 'vehicleType' is set
    if (vehicleType == null) {
      throw new ApiException(
          "Missing the required parameter 'vehicleType' when calling getVehicles(Async)");
    }
  ...
  public ApiResponse<VehiclesResponse> getVehiclesWithHttpInfo(
      ...
      throws ApiException, LWAException {
    okhttp3.Call call =
        getVehiclesValidateBeforeCall(
            marketplaceId, vehicleType, pageToken, updatedAfter, null, null);
