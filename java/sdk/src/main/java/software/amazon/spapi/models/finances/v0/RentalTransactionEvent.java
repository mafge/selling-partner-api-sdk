...
public class RentalTransactionEvent {
  ...
  /**
   * The type of rental event. Possible values: * RentalCustomerPayment-Buyout - Transaction type
   * that represents when the customer wants to buy out a rented item. *
   * RentalCustomerPayment-Extension - Transaction type that represents when the customer wants to
   * extend the rental period. * RentalCustomerRefund-Buyout - Transaction type that represents when
   * the customer requests a refund for the buyout of the rented item. *
   * RentalCustomerRefund-Extension - Transaction type that represents when the customer requests a
   * refund over the extension on the rented item. * RentalHandlingFee - Transaction type that
   * represents the fee that Amazon charges sellers who rent through Amazon. *
   * RentalChargeFailureReimbursement - Transaction type that represents when Amazon sends money to
   * the seller to compensate for a failed charge. * RentalLostItemReimbursement - Transaction type
   * that represents when Amazon sends money to the seller to compensate for a lost item.
   *
  ...
  @Schema(
      description =
          "The type of rental event.  Possible values:  * RentalCustomerPayment-Buyout -"
              + " Transaction type that represents when the customer wants to buy out a rented"
              + " item.  * RentalCustomerPayment-Extension - Transaction type that represents when"
              + " the customer wants to extend the rental period.  * RentalCustomerRefund-Buyout -"
              + " Transaction type that represents when the customer requests a refund for the"
              + " buyout of the rented item.  * RentalCustomerRefund-Extension - Transaction type"
              + " that represents when the customer requests a refund over the extension on the"
              + " rented item.  * RentalHandlingFee - Transaction type that represents the fee that"
              + " Amazon charges sellers who rent through Amazon.  *"
              + " RentalChargeFailureReimbursement - Transaction type that represents when Amazon"
              + " sends money to the seller to compensate for a failed charge.  *"
              + " RentalLostItemReimbursement - Transaction type that represents when Amazon sends"
  ...
  /**
   * The number of days that the buyer extended an already rented item. This value is only returned
   * for RentalCustomerPayment-Extension and RentalCustomerRefund-Extension events.
   *
  ...
  @Schema(
      description =
          "The number of days that the buyer extended an already rented item. This value is only"
              + " returned for RentalCustomerPayment-Extension and RentalCustomerRefund-Extension"
              + " events.")
  ...
  public RentalTransactionEvent rentalTaxWithheldList(
