...
public class TokensApi {
  ...
  private okhttp3.Call createRestrictedDataTokenValidateBeforeCall(
    ...
    okhttp3.Call call =
        createRestrictedDataTokenCall(body, progressListener, progressRequestListener);
    return call;
  }

  /**
   * Returns a Restricted Data Token (RDT) for one or more restricted resources that you specify. A
   * restricted resource is the HTTP method and path from a restricted operation that returns
   * Personally Identifiable Information (PII), plus a dataElements value that indicates the type of
   * PII requested. See the Tokens API Use Case Guide for a list of restricted operations. Use the
   * RDT returned here as the access token in subsequent calls to the corresponding restricted
   * operations. **Usage Plan:** | Rate (requests per second) | Burst | | ---- | ---- | | 1 | 10 |
   ...
   * that you specify. A restricted resource is the HTTP method and path from a restricted operation
   * that returns Personally Identifiable Information (PII), plus a dataElements value that
   * indicates the type of PII requested. See the Tokens API Use Case Guide for a list of restricted
   * operations. Use the RDT returned here as the access token in subsequent calls to the
   * corresponding restricted operations. **Usage Plan:** | Rate (requests per second) | Burst | |
   * ---- | ---- | | 1 | 10 | The &#x60;x-amzn-RateLimit-Limit&#x60; response header returns the
  ...
  public okhttp3.Call createRestrictedDataTokenAsync(
    ...
    okhttp3.Call call =
        createRestrictedDataTokenValidateBeforeCall(
