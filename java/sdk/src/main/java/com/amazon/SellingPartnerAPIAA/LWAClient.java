...
class LWAClient {
  ...
  String getAccessTokenFromEndpoint(LWAAccessTokenRequestMeta lwaAccessTokenRequestMeta)
    ...
    RequestBody requestBody =
        RequestBody.create(JSON_MEDIA_TYPE, new Gson().toJson(lwaAccessTokenRequestMeta));
    ...
    try {
      ...
      if (body == null)
        throw new LWAException(
            LWAExceptionErrorCode.other.toString(),
      ...
      if (!response.isSuccessful()) {
        ...
        if (responseJson.has("error")
            && EnumUtils.isValidEnum(
                LWAExceptionErrorCode.class, responseJson.get("error").getAsString())) {
          throw new LWAException(
